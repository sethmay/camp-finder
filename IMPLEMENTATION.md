...
- `filter.ts`: `filterCamps(camps, criteria)` — a **pure function** applying:
  ...
  - **features**: camp features **expanded up the vocab `broader` hierarchy** (`expandFeatures`) ⊇ the selected facet set — so a camp listing only `kayaking` matches the `aquatics` facet. The filter chips (`FEATURE_FACETS`) are a curated ~20-code **facet subset** of the 128-term open vocab, not the whole vocab, and are grouped into category subheadings (Activities / Programs & audience / Camp facilities & lodging) derived from each term's vocab `category`. `features_signature` (headline draws) and `features_verified_at` (survey date) are display-only, never filter inputs;
